...
- 收到每一条用户消息后，先追加当天日报再继续答复或执行；此规则永久生效，不依赖用户再次提醒。
...
- 每次打包前必须先将本次代码、版本号和设计文档变更提交并推送到 GitHub；打包成功后还必须把对应的构建完成记录、版本状态和日报再次提交并推送，确保 GitHub 同时保留可复现的打包前源码状态和已验证的打包后状态。
