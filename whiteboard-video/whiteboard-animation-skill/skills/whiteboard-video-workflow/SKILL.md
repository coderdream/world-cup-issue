...
运行本 skill 的 `scripts/check_env.py`，一次性检查所有依赖（Python 虚拟环境、OpenAI 兼容网关配置）：
...
> 白板动画视频生成需要 OpenAI 兼容 Images API 来调用 AI 模型先生成图片。你需要在 skill 目录下的 `.env` 文件中配置 `url`、`model`、`key`。
>
> 修复方法：
> 1. 在 `<skill-dir>/.env` 文件中添加三行：`url=http://81.68.73.15:3000/openai/v1`、`model=gpt-5.5`、`key=你的API密钥`
> 2. 如果文件不存在，先创建它
> 3. 也可以使用等价环境变量：`OPENAI_API_BASE`/`OPENAI_BASE_URL`、`OPENAI_IMAGE_MODEL`/`OPENAI_MODEL`、`OPENAI_API_KEY`/`CODEX_API_KEY`
> 4. 配置完成后重新运行即可
...
如果目标是“30 分钟读一本书”类现实感读书视频配图，先设置：

```bash
BOOK_IMAGE_PROMPT_STYLE=book-realistic
```

此时 `gen-prompts` 会生成适配 MacMini4 Realistic Vision 服务的中远景电影感分镜提示词，不再使用白板线稿风格。

如果用户更强调“贴合书本内容和一致性”，不要求真人照片，可设置：

```bash
BOOK_IMAGE_PROMPT_STYLE=book-illustration
```

此时会生成统一故事书插画/2D 动画电影 still 风格提示词，更强调信件、母女关系、旧信、离别和修复等书本主题。
...
读书视频现实感配图模式下，步骤 5 必须使用 MacMini4 图片服务环境变量：
...
在家里或跨网段时将 `MACMINI_IMAGE_ENDPOINT` 改为 `http://100.96.199.26:30020/v1/images/generations`。
...
- `check_env.py` - 一次性环境预检，检查 Python 虚拟环境和 OpenAI 兼容网关配置，自动安装可修复的依赖
...
- `generate-image.py` - 使用 OpenAI 兼容 Images API 文生图，支持单张和批量并发模式
- `prompt_template.py` - 白板风格提示词模板
## 故障判断

- `proxy` 只是可选项，用来让本机穿过代理访问网关，不是核心修复点。
- 如果 `images/generations` 返回 404，或者 `chat/completions` 只回 `data: [DONE]`，说明中转平台还没做到完整 OpenAI 兼容。
- 这种情况下脚本会先尝试 chat JSON fallback，再退回本地白板渲染；真正需要修的是中转平台的 `/openai/v1/images/generations` 和 `/openai/v1/chat/completions` 兼容性。
