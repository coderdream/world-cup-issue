...
SKIP_RATE = 1  # 每帧推进的网格步数基准；越大绘制越快，但运动更跳跃
HAND_SMOOTHING = 0.28  # 手部位置缓动系数，降低逐格跳跃带来的抖动
...
def smooth_hand_position(variables, target_x, target_y):
    """Ease the hand toward the next drawing target so grid jumps are less visible."""
    previous = variables.get("hand_position")
    if previous is None:
        smoothed_x = float(target_x)
        smoothed_y = float(target_y)
    else:
        prev_x, prev_y = previous
        smoothed_x = prev_x + (float(target_x) - prev_x) * HAND_SMOOTHING
        smoothed_y = prev_y + (float(target_y) - prev_y) * HAND_SMOOTHING
    variables["hand_position"] = (smoothed_x, smoothed_y)
...
def draw_masked_object(variables, target_cells, skip_rate=SKIP_RATE):
    ...
    for row, col in draw_order:
        ...
        if variables["draw_hand"]:
            hand_coord_x, hand_coord_y = smooth_hand_position(
                variables,
                range_h_start + split_len // 2,
                range_v_start + split_len // 2,
            )
...
def colorize_animation(
    ...
    for row, col in draw_order:
        ...
        if variables["draw_hand"]:
            hand_x, hand_y = smooth_hand_position(variables, cx, cy)
            drawn_frame_with_hand = draw_hand_on_img(
