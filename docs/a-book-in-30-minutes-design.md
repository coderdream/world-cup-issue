...
图片生成新增本机 187 / Y9000P ComfyUI 后端。只有显式设置 `BOOK_IMAGE_BACKEND=xiaohei-ai-y9000p` 时，脚本才读取 `Y9000P_COMFYUI_BASE_URL=http://127.0.0.1:8188`、`Y9000P_COMFYUI_WORKFLOW`、`Y9000P_COMFYUI_CHECKPOINT`、`Y9000P_COMFYUI_INPUT_DIR`、`Y9000P_COMFYUI_WIDTH`、`Y9000P_COMFYUI_HEIGHT`、`Y9000P_COMFYUI_STEPS`、`Y9000P_COMFYUI_CFG`、`Y9000P_COMFYUI_DENOISE`、`Y9000P_COMFYUI_RESTORE_GUIDE_LINE_ART`、`Y9000P_COMFYUI_GUIDE_CLEANUP_RADIUS`、`Y9000P_COMFYUI_REQUEST_TIMEOUT_SECONDS`、`Y9000P_COMFYUI_MAX_WAIT_SECONDS` 和 `Y9000P_COMFYUI_POLL_SECONDS`。该后端默认使用官方风格受控 img2img：先用本机程序化小黑分镜生成 32~64 张有中文标注的 `xiaohei_ai_y9000p_guides/guide_XX.png`，画法对齐官方样例的白底、黑色小人、稀疏概念隐喻、红蓝橙箭头和短中文标注；同时生成无中文标注的 `guide_XX_no_text.png` 并缩放复制到 `D:\AI\apps\ComfyUI\input\xiaohei_y9000p_guides`，通过 `LoadImage -> VAEEncode -> KSampler -> VAEDecode -> SaveImage` 调用本机 ComfyUI 精修。默认 checkpoint 为 `DreamShaper8_LCM.safetensors`，默认 `sampler=lcm`、`scheduler=sgm_uniform`、`steps=8`、`cfg=1.7`、`denoise=0.28`，用于保持构图、字幕安全区和小黑角色稳定；显式设置 `Y9000P_COMFYUI_WORKFLOW=txt2img` 时才回到旧的自由生图 smoke 路线。由于扩散模型会破坏中文标注，模型输入刻意不包含中文，最终视频图默认开启 `Y9000P_COMFYUI_RESTORE_GUIDE_LINE_ART=1`，把有中文 guide 的非白色线稿、箭头和中文重新覆盖到 AI 输出上，保证文字和图意可控；默认 `Y9000P_COMFYUI_GUIDE_CLEANUP_RADIUS=5` 用于覆盖前轻微擦除 guide 区域，避免伪线条重影。输出写入 `xiaohei_ai_y9000p` 目录，同时写入 `xiaohei_ai_y9000p_manifest.json` 和 `visual_assets_manifest.json`，manifest 记录 guide、ComfyUI raw 输出、最终视频图、prompt、工作流参数和每张图的分镜元数据。若本机 ComfyUI 不可用或生成失败，流水线会在 stderr 写明原因并回退到 `xiaohei-sequence`，避免图片阶段中断。
...
`xiaohei-ai-y9000p` is available as an explicit local AI image backend. It calls the Y9000P / 187 ComfyUI node at `http://127.0.0.1:8188` by default, now using official-style controlled img2img rather than free txt2img: the pipeline renders sparse programmatic Xiaohei guide images, copies textless resized guides into `D:\AI\apps\ComfyUI\input\xiaohei_y9000p_guides`, and runs DreamShaper LCM at low denoise so the GPU refines line texture without learning or inventing Chinese text. Because the model corrupts Chinese annotations, the final image restores the labeled guide line art over the AI output by default. Raw ComfyUI outputs are written to `xiaohei_ai_y9000p`, final video images are resized to 1920x1080, and the manifest records `sourceKind=xiaohei_ai_y9000p`, `workflow=controlled-img2img`, `restoreGuideLineArt`, guide paths, prompts, and generation parameters. Set `Y9000P_COMFYUI_WORKFLOW=txt2img` only for the older free-generation smoke path.
