import { Bot, Clipboard, KeyRound, MessageCircle, RefreshCw, Send, Settings, Sparkles } from "lucide-react";
...
const defaultPrompt = "请用三句话说明这个 Tauri 框架适合承载哪些桌面应用能力。";
...
export function SettingsPage() {
  ...
  return (
    <div className="page">
      ...
      <Panel>
        <SectionTitle icon={<KeyRound size={16} />} title="接口配置" inline />
        <label className="field">
          <span>API Base URL</span>
          <input value={settings.apiBaseUrl} onChange={(event) => void updateSettings({ apiBaseUrl: event.target.value })} />
        </label>
        <label className="field">
          <span>API Key</span>
          <input type="password" value={settings.apiKey} onChange={(event) => void updateSettings({ apiKey: event.target.value })} />
        </label>
      </Panel>
