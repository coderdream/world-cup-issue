...
# === 固定算法参数 ===
FRAME_RATE = 30  # 输出视频帧率；1080p 全片渲染用 30fps 平衡清晰度和耗时
SPLIT_LEN = 10  # 网格切分边长（像素）；越小绘制越细，计算量越大
MAX_1080P = False  # 本次渲染使用固定 1920x1080，不按最长边压到 1080
...
TARGET_WIDTH = 1920  # 本次完整视频固定输出宽度
TARGET_HEIGHT = 1080  # 本次完整视频固定输出高度
...
def main():
    ...
    # 计算目标分辨率。完整成片固定为 1920x1080，避免默认最长边 1080 导致字幕版看不清。
    if MAX_1080P:
        max_dim = 1080
